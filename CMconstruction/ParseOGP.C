...
const bool print_aahana_list=true;
const bool print_stripe_coords=true;
const float delR_warning_threshold=1.5;
...
void ParseOGP(int q){
  ...
  // and the survey marks too
  const int MAXPETALS=100;
  const int MAXROWS=213;
  const int MAXSURVEY=4;
  ...
  std::array<bool,MAXPETALS> petalExists;
  std::array<std::array<TVector2,MAXSURVEY>,MAXPETALS> surveyPos,srelPos,slocalPos,slocalRotPos,srelRotPos;
  ...
  for (int i=0;i<MAXPETALS;i++){
    ...
    surveyTree->Draw("i:pos.X():pos.Y()",Form("petal==%d",i),"goff");
    nRows=surveyTree->GetSelectedRows();
    printf("parsing survey %d (nrows=%d)\n",i,nRows);
    for (int j=0;j<nRows;j++){
      surveyPos[i][j].Set(surveyTree->GetVal(1)[j],surveyTree->GetVal(2)[j]);
    }
  }
  ...
  for (int i=0;i<MAXPETALS;i++){
    ...
    //conversion requires that the survey marks are also aligned to petal zero, not to survey mark zero
    for (int j=0;j<MAXSURVEY;j++){
      slocalPos[i][j]=surveyPos[i][j]-stripePos[i][0];
      srelPos[i][j]=surveyPos[i][j]-stripePos[i][0]-(surveyPos[basepetal][j]-stripePos[basepetal][0]);
    }
  }

 
  //interlude:  print a preamble if we are outputting lines of a gnuplot macro
  if (build_gnuplot){
  ...
  //interlude:  setup some histograms to track the data
  ...
  for (int i=0;i<MAXPETALS;i++){
    if (!petalExists[i]) continue; //skip petals that aren't real;
    TVector2 axis=localPos[i][212]; //farthest away.  its position is wrt stripe zero by construction.
    ...
    //rotate survey marks as well.
    for (int j=0;j<MAXSURVEY;j++){
     slocalRotPos[i][j]=slocalPos[i][j].Rotate(-phi);
      srelRotPos[i][j]=slocalRotPos[i][j]-slocalRotPos[basepetal][j];
     }
  ...
  for (int i=0;i<MAXPETALS;i++){
    ...
    for (int j=0;j<MAXSURVEY;j++){
      slocalRotPos[0][j]=slocalRotPos[0][j]+slocalRotPos[i][j];
    }
  }
   for (int j=0;j<MAXROWS;j++){
  ...
  // if (!build_gnuplot) hArea0to212->Draw("colz");
   ...
   if (print_aahana_list){
     int petal=44;
     printf("petal %d\n",petal);
     printf("survey marks\n");
     for (int j=0;j<4;j++){
       printf("%d,%2.2f,%2.2f\n",
	      j,slocalRotPos[petal][j].X(),slocalRotPos[petal][j].Y());
     }
     printf("stripes\n");
     for (int j=0;j<MAXROWS;j++){
       printf("%d,%2.2f,%2.2f\n",
	      j,localRotPos[petal][j].X(),localRotPos[petal][j].Y());
     }
     return;
   }
     ...
     if (build_gnuplot){
       for (int j=0;j<MAXROWS;j++){
	 printf("set label %d \"%d\" at %1.3f,%1.3f center norotate font \"Verdana,30\" front\n",
		j+1,j,localRotPos[0][j].X()-localRotPos[0][0].X(),localRotPos[0][j].Y()-localRotPos[0][0].Y()+5);
       }
      ...
      printf("#petal %d\n",i);
      //make circles around each bad stripe
         for (int j=0;j<MAXROWS;j++){
	   if (j==2) continue; //skip the one that's missing the area
	   if (relArea[i][j]<0.9 || relArea[i][j]>1.25){
	     //printf("Petal %d stripe %d is bad (relArea=%f, absAre=%f\n",i,j,relArea[i][j],stripeArea[i][j]);
	     printf("set object %d circle at %1.3f,%1.3f size first %d fc rgb \"red\" lw 15\n",j,localRotPos[0][j].X()-localRotPos[0][0].X(),localRotPos[0][j].Y()-localRotPos[0][0].Y()-2,j<72?6:12);
	   }
	   if (relRotPos[i][j].Mod()>delR_warning_threshold){
	     ...
	     printf("set object %d circle at %1.3f,%1.3f size first %d fc rgb \"orange\" lw 15\n",j,localRotPos[0][j].X()-localRotPos[0][0].X(),localRotPos[0][j].Y()-localRotPos[0][0].Y()-2,j<72?6:12);
      ...
      printf("plot  \"<grep 'Circle     7' '%spetal%d_edges.DAT' -A 350000\" u (($1+dx%d)*cos(dphi%d)-($2+dy%d)*sin(dphi%d)):(($2+dy%d)*cos(dphi%d)+($1+dx%d)*sin(dphi%d)) w l title 'petal %d'\n",directory.c_str(),i,
	     i,i,i,i,i,i,i,i,i);
          printf("unset for [i=0:250] object i\n\n");

...
const int resample_factor=10; //how many samples, evenly spaced but centered in the bin boundaries so that they're never on the edge unless n=inf, to resample.  may have aliasing issues if we're not careful.
...
void Resample(std::vector<TH3*> hin, std::vector<TH3*> hout){
  ...
  for (int i=0;i<nbins[0];i++){
	...
	hhits->Fill(distorted_pos[0],distorted_pos[1],distorted_pos[2],1);
	for (int m=0;m<3;m++){
	  int global_bin=hin[m]->FindBin(distorted_pos[0],distorted_pos[1],distorted_pos[2]);
	  hout[m]->SetBinContent(global_bin,hout[m]->GetBinContent(global_bin)/hhits->GetBinContent(global_bin));
	}     
      }
    }
  }

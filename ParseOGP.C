...
void ParseOGP(){
  ...
  TFile *petalFile=TFile::Open("/Users/rcorliss/Desktop/Output Files.3/all_petals.hist.root","RECREATE");
  ...
  for (int petalIndex=0;petalIndex<95;petalIndex++){
    txtfile.open(Form("/Users/rcorliss/Desktop/Output Files.3/petal%d_edges.txt",petalIndex),ios::in);   
	...
	if (tp.find(GetHeaderLine(readerstep+adjust))!=std::string::npos){
	  ...
	  if (readerstep<11) {
	    ...
	    if (petalIndex==70 && readerstep==171){ adjust++; //step 172 outlines more of stripe 171, but does not write to the text file as of July 29;
	      printf(">>>>>>>>>>>>>>>adjusting after step 171 for petal 70\n");
	    }
...
void ParseCircles(){
  ...
  TFile *petalFile=TFile::Open("/Users/rcorliss/Desktop/Output Files.3/all_petals.hist.root","READ");
...
void ParseOGP(int q){
  ...
  for (int i=0;i<MAXPETALS;i++){
    ...
    for (int j=0;j<MAXROWS;j++){
      if (j==2) continue; //skip the one that's missing the area
      int badSize=0;
      if (relArea[i][j]<0.85 || relArea[i][j]>1.2){
	badSize=2;
	stripeBadSize[j]++;
	nBadSize++;

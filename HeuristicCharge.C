...
model_params Sphenix2020ModelParams(){
  ...
  float ion_mobility=1.65;//[cm/s*cm/V] ion mobility in 50:50 Ne:CF4 gas;
  ...
  float ions_per_cm=71.5;//[ions/cm] approximate ions per cm in 50:50
  float kev_per_cm=4.28;//[kev/cm] mip energy loss in 50:50
  ...
  double gain=1410;//originally 2000, scaled so that total gain in the old 90:10 is approximately the same as total gain here.;
  double ibf_per_gain=0.0067;//0.67% IBF, 2 ions escaping back per every 300 electrons in the shower.
   float c=gain*ibf_per_gain;//gain of 2k, 0.67% IBF per unit gain.
  a=a*1e15; //fC/m
  a=a/100;//fC/cm
  //rho= a/r^2*(1-bz+c)
...
model_params Sphenix2018ModelParams(){
  ...
  float b=1.0/z_rdo; //[1/cm]
   float c=(0.3/100.0)*2000.0;//gain of 2k, 0.67% IBF per unit gain.
  a=a*1e15; //fC/m
  a=a/100;//fC/cm
  //rho= a/r^2*(1-bz+c)

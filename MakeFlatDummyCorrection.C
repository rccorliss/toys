...
void MakeFlatDummyCorrection(const char* filebase="dummyCorrection", const float rval=0,const float phival=0, const float zval=0){
  ...
  for (int i=0;i<2;i++){
    for (int j=0;j<3;j++){
      hFlat[i][j]=new TH3F(Form("hIntDistortion%s%s",axis[j].c_str(),extension[i].c_str()),
  ...
  TFile *output=TFile::Open(Form("%s.hist.root",filebase),"RECREATE");
  for (int i=0;i<2;i++){
    for (int j=0;j<3;j++){
      hFlat[i][j]->Write();
    }
